...
//  ---------------------------------------------------------------------------
//  Les contraintes liÃ©es Ã  l'application RaceBox officielle ont Ã©tÃ© retirÃ©es :
//  le nom annoncÃ©, le modÃ¨le et le numÃ©ro de sÃ©rie sont dÃ©sormais libres.
//  Seul le FORMAT DES TRAMES reste inchangÃ© (en-tÃªte UBX, message de donnÃ©es
//  de 80 octets, commandes 0xFF 0x2x) : c'est lui que la console web utilise,
//  et le modifier casserait tout le reste.
// ============================================================================

// --- PSEUDO : personnalisez cette ligne pour chaque module. C'est ce nom qui
//     apparaÃ®t lors de la recherche Bluetooth, ce qui permet de distinguer
//     plusieurs appareils allumÃ©s en mÃªme temps. Restez sous 16 caractÃ¨res :
//     au-delÃ , la trame d'annonce Bluetooth dÃ©borde et le nom est tronquÃ©.
#define DEVICE_NICKNAME "TrimVa"

#define SERIAL_NUM "0123456789" // identifiant libre, informatif seulement
#define BRAND       "DIY"
#define DEVICE_NAME "TrimBox " DEVICE_NICKNAME  // nom visible en Bluetooth
#define MODEL_STRING BRAND
#define HARDWARE_VER "1.0"
#define MANUFACTURER BRAND
#define FIRMWARE_VER "1.0"

// --- AJOUT : empreinte de compilation. Le bootloader UF2 redÃ©marre la carte
//     dÃ¨s le dernier bloc Ã©crit, ce qui provoque une erreur de dÃ©connexion
//     cÃ´tÃ© ordinateur ou tÃ©lÃ©phone : celle-ci est normale et n'indique pas si
//     le firmware a rÃ©ellement Ã©tÃ© pris. Cette empreinte, affichÃ©e au
//     dÃ©marrage ET dans le rapport pÃ©riodique, lÃ¨ve le doute.
#define BUILD_STAMP __DATE__ " " __TIME__

// --- GPS Performance ---
#define MAX_NAVIGATION_RATE 25 // 25 Hz : cadence maximale du format de trame
...
#define ENABLE_DEEP_SLEEP false   // Ã  laisser Ã  false en usage courant
...
// ###  Format de trame hÃ©ritÃ© de la documentation du protocole BLE        ###
// ###  RaceBox (rÃ©vision 8), conservÃ© tel quel car la console web s'y      ###
// ###  appuie. Aucune contrainte de compatibilitÃ© applicative ne subsiste. ###
...
// --- Types d'enregistrement stockÃ©s (= identifiant du message) ---
...
static void defaultConfig() {
  ...
  recCfg.flags = 0x1F;          // tous les filtres actifs
  recCfg.statSpeed = 1389;      // ~5 km/h
  recCfg.statInterval = 30;
  recCfg.noFixInterval = 30;
  recCfg.autoOffInterval = 300; // 5 minutes
  // 7 = DYN_MODEL_AIRBORNE2g : rÃ©glage "piste", tolÃ©rant aux accÃ©lÃ©rations
  // fortes. La console peut le changer via la commande 0xFF 0x27.
...
static void handleCommand(uint8_t cls, uint8_t id, uint8_t *pl, uint16_t len) {
  ...
  // --- 0xFF 0xF0 : identification du firmware (extension propriÃ©taire) ---
  // Extension maison, absente du format d'origine : un appareil tiers
  // rÃ©pondrait par un refus, ce que la console interprÃ¨te sans erreur.
  case 0xF0: {
    char info[128];
    int n = snprintf(info, sizeof(info), "%s|%s|%s|%u|%s",
...
// Assemble le message de donnÃ©es de 80 octets
...
void setupAdvertising(int8_t power, uint16_t interval) {
  ...
  // Les UUID de service restent dans l'annonce principale : c'est ainsi que
  // la console web filtre les appareils lors de la recherche.
...
void enableIMU() {
  ...
  // --- MODIFIÃ‰ : Â±16 g au lieu de Â±8 g. Sur une voiture RC, les chocs
  //     d'atterrissage de saut saturaient l'Ã©chelle Â±8 g : les relevÃ©s
  //     plafonnaient alors Ã  7,99 g sur les trois axes simultanÃ©ment, ce qui
  //     masquait la valeur rÃ©elle. La contrepartie est une rÃ©solution deux
  //     fois moins fine (0,488 mg par pas au lieu de 0,244), nÃ©gligeable
  //     devant l'amplitude mesurÃ©e ici.
...
void reportSystemStats() {
  ...
  Serial.printf("BUILD   | %s Â« %s Â» Â· %s Â· %s Â· accel Â±16 g\n",
...
void setupBLE() {
  ...
  disModel.begin();
  // Nom de modÃ¨le, lu par la console pour identifier l'appareil.
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n\nðŸš€ SYSTEM STARTUP");
  Serial.printf("   %s Â« %s Â» Â· firmware %s Â· compilÃ© le %s\n",
                BRAND, DEVICE_NICKNAME, FIRMWARE_VER, BUILD_STAMP);
  Serial.println("   Commandes sÃ©rie : s=stop  i=Ã©tat  z=config par dÃ©faut  ?=aide");
